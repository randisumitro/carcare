...
<body>
  <main class="bg-[#FAFAFA] max-w-[640px] mx-auto min-h-screen relative flex flex-col has-[#CTA-nav]:pb-[120px] has-[#Bottom-nav]:pb-[120px]">
      ...
      <div class="flex flex-col w-full rounded-2xl border border-[#E9E8ED] p-4 gap-4 bg-white">
        ...
        <div id="Price-details" class="flex flex-col gap-[10px]">
          ...
          <div class="flex items-center justify-between">
            ...
            <p class="font-semibold">Rp {{ number_format($carService->totalPpn, 0, ',', '.') }}</p>
          </div>
          <div class="flex items-center justify-between">
    ...
    <div class="flex h-full flex-1 mt-5">
      <form action="{{ route('front.booking.payment.store') }}" method="POST" enctype="multipart/from-data" class="w-full flex flex-col rounded-t-[30px] p-5 pt-[30px] gap-[26px] bg-white overflow-x-hidden mb-0 mt-auto">
        @csrf
