@extends('front.layouts.add')
@section('title', 'Cuci mobil randi')
@section('content')
            ...
            <div class="rounded-[20px] border border-[#E9E8ED] py-4 flex flex-col items-center text-center gap-4 bg-white transition-all duration-300 hover:ring-2 hover:ring-[#FF8E62]">
              ...
              <div class="flex flex-col">
                ...
                <p class="text-xs leading-[18px] text-[#909DBF]">
                  {{ $service->storeServices->count() }}Store</p>
              </div>
            </div>
          </a>
          @empty
...
<script>
  ...
  link.addEventListener('click', function(e) {
    ...
    // Ambil nilai dari elemen dengan ID 'city_id'
    const cityId = document.getElementById('city_id').value;

    // Ambil atribut 'data-service' dari elemen yang di-klik
    const serviceTypeId = this.getAttribute('data-service');
    
    // Redirect ke route dengan parameter yang sesuai
